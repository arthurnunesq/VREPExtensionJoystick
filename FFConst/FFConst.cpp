...
int main() {
	std::cout << "Force feedback control application" << std::endl;

	HWND handle = nullptr;
	handle = FindWindowA(NULL, "E:\\OneDrive\\Projetos\\VREPExtensionJoystick\\Debug\\FFConst.exe");
	if (!handle) {
		printf("Could not get window handle.\n");
	}

	InitDirectInput(handle);//nullptr);

	// Make sure the device is acquired, if we are gaining focus.
	g_pDevice->Acquire();
	
	if (g_pEffect) {
		if (FAILED(g_pEffect->Start(1, 0))) { // Start the effect
			printf("Failed at 'g_pEffect->Start'.\n");
		}

		g_nXForce = DI_FFNOMINALMAX / 4;
		g_nYForce = DI_FFNOMINALMAX / 4;

		if (FAILED(SetDeviceForcesXY())) {
			printf("Failed at 'SetDeviceForcesXY'.\n");
		}
...
HRESULT InitDirectInput( HWND hDlg )
{
    ...
    {
		printf("Failed at 'DirectInput8Create'.\n");
        return hr;
    }
    ...
    {
		printf("Failed at 'EnumDevices'.\n");
        return hr;
    }

    if( !g_pDevice )
    {
		printf("Failed at invalid device handler.\n");
    ...
    if( FAILED( hr = g_pDevice->SetCooperativeLevel(
		//nullptr,
		hDlg,
	...
	if (FAILED(hr = g_pDevice->SetProperty(DIPROP_AUTOCENTER, &dipdw.diph))) {
		printf("Failed at 'SetProperty(DIPROP_AUTOCENTER,...)'.\n");
		return hr;
	}
	...
	if (!g_pEffect) {
		printf("Failed at invalid effect handler.\n");
		return E_FAIL;
	}
